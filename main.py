...
def main(args):
    ...
    # Do git checks
    pkg_gitdict = mbatch.gitcheck(config,'gitcheck_pkgs',package=True)
    pth_gitdict = mbatch.gitcheck(config,'gitcheck_paths',package=False)
            ...
            if (type(cstages[stage]['arg'])) in [list,tuple]:
                ...
                for k,arg in enumerate(cstages[stage]['arg']):
                    ...
                    if new_stage_name in stage_names:
                        mbatch.raise_exception(f"Internal loop stage name "
    ...
    for sname in stage_names:
        ...
        for d in ds:
            if not(d in stage_names) and not(d in unroll_map):
                mbatch.raise_exception(f"Stage {d} required by {sname} not found in configuration file.")
            if d in unroll_map:
    ...
    if mbatch.has_loop(deps): mbatch.raise_exception("Circular dependency detected.")
    stages = mbatch.flatten(deps) # Ordered by dependency
    ...
    if set(stages)!=set(stage_names): mbatch.raise_exception("Internal error in arranging stages. Please report this bug.")

    # Check sanity of skipped stages list
    if args.skip is None: args.skip = []
    for skipstage in args.skip:
        if not(skipstage in stages): mbatch.raise_exception("Asked to skip a stage that is not in the list of stages.")
    ...
    if not(args.no_reuse):
        ...
        for stage in stages:
            ...
            root = mbatch.get_out_file_root(root_dir,stage,args.project,site) + "_"
            suffix = ".txt"
            ...
            if len(fs)==0:
                ...
                output = mbatch.run_local(['sacct', '-j',
                ...
                for i,line in enumerate(output):
                    if i==0:
                        if line.strip()!='State': mbatch.raise_exception("Unexpected output from sacct.")
                    elif i==(len(output)-1):
                        if line.strip()!='': mbatch.raise_exception("Unexpected output from sacct.")
                    else:
            ...
            try:
                with open(mbatch.get_stage_config_filename(root_dir,stage,args.project), 'r') as stream:
    ...
    if sum([int(x) for x in [is_sbatch,is_local]])!=1: mbatch.raise_exception("Inconsistency in submission vs. local. Report bug.")
    ...
    if reply in ['Y','YES']:
        ...
        mbatch.raise_exception("Invalid input.")

        
    # Make project directory
    proj_dir = mbatch.get_project_dir(root_dir,args.project)
    ...
    for stage in stages:
        ...
        # Get command
        execution,script,pargs = mbatch.get_command(global_vals, copy.deepcopy(ostages), stage)

        # Make output directory
        output_dir =  mbatch.get_output_dir(root_dir,stage,args.project)
        ...
        if is_sbatch:
            jobid = mbatch.submit_slurm(stage,sbatch_config,
        ...
        if is_local:
            ...
            jobid = mbatch.run_local(cmds,dry_run=args.dry_run)
        ...
        if not(args.dry_run):
            ...
            with open(mbatch.get_stage_config_filename(root_dir,stage,args.project), 'w') as f:
...
if __name__ == '__main__':
    ...
    if args.force_local and args.force_slurm: mbatch.raise_exception("You can\'t force both local and SLURM.")
    main(args)
